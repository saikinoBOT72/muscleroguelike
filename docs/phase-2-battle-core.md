...
- `evade` は「攻撃1回」を無効化。`攻撃4×2` は 1ヒットのみ無効化する
  - 仕様書の「サイドステップ+ = 2ターン持続」は、**回数制(2回無効化)**として実装する。
    持続ターン制だと0コストのBASICカードとしては強すぎるため。`spec-master.md` も同様に修正済み
- **`evade` / `counter` は「次の敵ターンを守るための」フラグ**なので、`endPlayerTurn()` では消さず、
  次の `startPlayerTurn()` の先頭でリセットする(使い切らなかった分は持ち越さない)
- 敵のブロックは **敵ターンの開始時**にリセットする。
  これにより「敵がブロックを張る → プレイヤーのターン中は有効 → 敵の次のターンで消える」となる
- 手札の上限は **10枚**。超えて引こうとした場合はドローを行わずログに出す
